...
impl ServerHandler for ZhiServer {
    ...
    async fn list_tools(
        ...
        // SkillsTool dynamic registration is disabled; avoid exposing skill_* tools.
    ...
    async fn call_tool(
        ...
        let result: Result<CallToolResult, McpError> = match tool_name.as_str() {
            ...
            name if name == "skill_run" || name.starts_with("skill_") => {
                // SkillsTool dynamic registration is disabled; skill_* tools are not exposed.
                Err(McpError::invalid_request(format!(
                    "{} is disabled. SkillsTool dynamic registration is disabled.",
                    name
                ), None))
            }
            "enhance" => {
